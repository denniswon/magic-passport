...
import {
    ...
    MODULE_TYPE_HOOK,
    MODULE_TYPE_MULTI,
    SUPPORTS_NESTED_TYPED_DATA_SIGN
...
contract Passport is IPassport, BaseAccount, ExecutionHelper, ModuleManager, UUPSUpgradeable {
    ...
    /// `Passport.installModule` should be included in `userOp.callData`.
    ...
    function initializeAccount(bytes calldata initData) external payable virtual {
        ...
        require(success, PassportInitializationFailed());
    ...
    /// @notice Checks the 1967 implementation slot, if not found then checks the slot defined by address (pre-upgrade smart account)
    ...
    /// 2. address() slot in case if it's potentially upgraded from another account,
    /// as proxy reads implementation from the slot that is defined by its address
