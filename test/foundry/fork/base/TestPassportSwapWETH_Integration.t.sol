...
/// @title TestPassportSwapWETH_Integration
/// @notice Tests Passport smart account functionalities with Uniswap V2 swaps using WETH
contract TestPassportSwapWETH_Integration is BaseSettings {
    ...
    /// @notice Tests gas consumption for swapping WETH for USDC using a deployed Passport account
    function test_Gas_Swap_DeployedPassport_SwapWethForTokens() public checkERC20Balance(preComputedAddress) {
        Passport deployedPassport = deployPassport(user, 10 ether, address(VALIDATOR_MODULE));

        // Approve WETH transfer for deployed Passport
        ...
        measureAndLogGas("48::UniswapV2::swapExactTokensForTokens::Passport::Deployed::N/A", userOps);
    }

    /// @notice Tests deploying Passport and swapping WETH for USDC with Paymaster
    ...
    /// @notice Tests deploying Passport and swapping WETH for USDC using deposit
    ...
    /// @notice Tests gas consumption for batch approval and swapping WETH for USDC using deployed Passport account
    function test_Gas_BatchApproveAndSwap_DeployedPassport() public checkERC20Balance(preComputedAddress) {
        Passport deployedPassport = deployPassport(user, 10 ether, address(VALIDATOR_MODULE));
        ...
        PackedUserOperation[] memory userOps = buildPackedUserOperation(user, deployedPassport, EXECTYPE_DEFAULT, executions, address(VALIDATOR_MODULE), 0);
        measureAndLogGas("51::UniswapV2::approve+swapExactTokensForTokens::Passport::Deployed::N/A", userOps);
    }

    /// @notice Tests deploying Passport and batch approval and swapping WETH for USDC with Paymaster
    ...
    /// @notice Tests deploying Passport and batch approval and swapping WETH for USDC using deposit
    ...
    /// @notice Tests sending ETH to the Passport account before deployment and then deploy with Uniswap V2 swap using WETH
    function test_Gas_Swap_DeployPassportWithPreFundedETH() public checkERC20Balance(preComputedAddress) {
    ...
    /// @notice Tests gas consumption for swapping WETH for USDC using a deployed Passport account with Paymaster
    function test_Gas_Swap_DeployedPassport_SwapWethForTokens_WithPaymaster()
        public
        checkERC20Balance(preComputedAddress)
        checkPaymasterBalance(address(paymaster))
    {
        ...
        measureAndLogGas("55::UniswapV2::swapExactTokensForTokens::Passport::WithPaymaster::N/A", userOps);
    }
