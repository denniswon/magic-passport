...
/// @title TestPassportSwapETH_Integration
/// @notice Tests Passport smart account functionalities with Uniswap V2 swaps
contract TestPassportSwapETH_Integration is BaseSettings {
    ...
    /// @notice Tests gas consumption for swapping ETH for USDC using a deployed Passport account
    function test_Gas_Swap_DeployedPassport_SwapEthForTokens() public checkERC20Balance(preComputedAddress, SWAP_AMOUNT) {
        ...
        Execution[] memory executions = prepareSingleExecution(
            ...
            abi.encodeWithSignature(
                "swapExactETHForTokens(uint256,address[],address,uint256)", 0, getPathForETHtoUSDC(), address(deployedPassport), block.timestamp
            )
        );
        ...
        measureAndLogGas("42::UniswapV2::swapExactETHForTokens::Passport::Deployed::N/A", userOps);
    }

    /// @notice Tests deploying Passport and swapping ETH for USDC with Paymaster
    ...
    function test_Gas_Swap_DeployAndSwap_WithPaymaster() public checkERC20Balance(preComputedAddress, SWAP_AMOUNT) checkPaymasterBalance(address(paymaster)) {
        ...
        PackedUserOperation[] memory userOps = buildPackedUserOperation(
            ...
            Passport(preComputedAddress), // Passport account precomputed address
    ...
    /// @notice Tests deploying Passport and swapping ETH for USDC using deposit
    ...
    /// @notice Tests sending ETH to the Passport account before deployment and then deploy with Uniswap V2 swap
    function test_Gas_Swap_DeployPassportWithPreFundedETH() public checkERC20Balance(preComputedAddress, SWAP_AMOUNT) {
    ...
    /// @notice Tests gas consumption for swapping ETH for USDC using a deployed Passport account with Paymaster
    function test_Gas_Swap_DeployedPassport_SwapEthForTokens_WithPaymaster()
        public
        ...
        checkPaymasterBalance(address(paymaster))
    {
        ...
        PackedUserOperation[] memory userOps = buildPackedUserOperation(
            user, // Wallet initiating the operation
            deployedPassport, // Deployed Passport account
            EXECTYPE_DEFAULT, // Execution type
            executions, // Execution details
            address(VALIDATOR_MODULE), // Validator module address
            0 // Nonce
        ...
        // Measure and log gas usage for the operation
        measureAndLogGas("46::UniswapV2::swapExactETHForTokens::Passport::WithPaymaster::N/A", userOps);
    }
