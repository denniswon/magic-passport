...
/// @title Gas benchmark tests for PassportAccountFactory
contract TestGas_PassportAccountFactory is TestModuleManagement_Base {
    ...
    function test_Gas_DeployAccount() public {
        ...
        assertValidCreation(Passport(newAccount));
    }
    ...
    function getInitData(address validator, address owner) internal view returns (bytes memory) {
        ...
        return BOOTSTRAPPER.getInitPassportScopedCalldata(validators, hook, REGISTRY, ATTESTERS, THRESHOLD);
    }

    /// @notice Validates the creation of a new account.
    /// @param _account The new account address.
    function assertValidCreation(Passport _account) internal {
