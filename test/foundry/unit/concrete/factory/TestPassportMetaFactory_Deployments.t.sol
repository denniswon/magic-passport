...
import "../../../../../contracts/factory/PassportMetaFactory.sol";
import "../../../../../contracts/factory/K1ValidatorFactory.sol";

/// @title PassportMetaFactory
/// @notice Tests for managing the factory whitelist and deploying accounts using the PassportMetaFactory.
contract TestPassportMetaFactory_Deployments is PassportTest_Base {
    Vm.Wallet public user;
    PassportMetaFactory public metaFactory;
    ...
    function setUp() public {
        ...
        metaFactory = new PassportMetaFactory(address(FACTORY_OWNER.addr));
        mockFactory = address(
            new K1ValidatorFactory(address(FACTORY_OWNER.addr), address(ACCOUNT_IMPLEMENTATION), address(VALIDATOR_MODULE), new PassportBootstrap(), REGISTRY)
        );
    }
    ...
    function test_Constructor_RevertIf_ZeroOwner() public {
        ...
        new PassportMetaFactory(address(0));
    }
